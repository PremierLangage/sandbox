...
from wrapt_timeout_decorator import timeout as add_timeout
...
class Command:
    ...
    def from_config(cls, config: dict) -> list["Command"]:
        ...
        if not config["commands"]:
            raise SuspiciousOperation("Command list cannot be empty")
        ...
        for c in config["commands"]:
            if isinstance(c, dict) and cls._check(c):
                ...
                raise SuspiciousOperation(f"Command badly formatted : '{c}'")
    ...
    def execute(self, container: Container) -> tuple[bool, dict]:
        ...
        start = time.time()
        try:
            exec_run = add_timeout(self.timeout, use_signals=False)(container.exec_run)
            exit_code, output = exec_run(
                ["bash", "-c", self.command], environment=self.environ, demux=True
            )
            stdout, stderr = (
                "" if out is None else out.decode().strip() for out in output
            )
        except TimeoutError:
            exit_code = SandboxErrorCode.TIMEOUT.value
            stdout = ""
            stderr = f"Command timed out after {self.timeout} seconds\n"
        except Exception:  # pragma: no cover
            logger.exception(
                f"An error occurred while executing the command '{self.command}'"
            )
            exit_code = SandboxErrorCode.UNKNOWN.value
            stdout = ""
        ...
        if (
            exit_code < 0 and exit_code != SandboxErrorCode.TIMEOUT.value
        ):  # pragma: no cover
            status = False
...
class Executor:
    ...
    def _move_env_to_container(self) -> None:
        ...
        logger.debug(
            f"Moving environment to container took : {time.time() - start} seconds"
        )
    ...
    def _get_result(self) -> str | None:
        ...
        start = time.time()
        with open(
            os.path.join(self.sandbox.envpath, self.result_path), encoding="UTF-8"
        ) as f:
            content = f.read()
        logger.debug(
            f"Getting result from container took : {time.time() - start} seconds"
        )
        return content
    ...
    def execute(self) -> dict:
        ...
        if self.result_path is not None:
            try:
                ...
                status = SandboxErrorCode.RESULT_NOT_FOUND.value
            except UnicodeDecodeError:
                status = SandboxErrorCode.RESULT_NOT_UTF8.value
