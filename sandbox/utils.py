...
def merge_tar_gz(a: Optional[BinaryIO], b: Optional[BinaryIO]) -> Optional[BinaryIO]:
    ...
    Returns
        ...
        io.BytesIO - A BytesIO, cursor set at the start, corresponding to the
                     ...
                     name)."""
    if a is None:
        return io.BytesIO(b.read()) if b is not None else b
    if b is None:
...
def get_env(env: str) -> Optional[str]:
    ...
    path = os.path.join(settings.ENVIRONMENT_DIR, env + ".tgz")
...
def executed_env(request: HttpRequest, config: dict) -> Optional[str]:
    ...
    if sandbox_env_uuid is not None:
        ...
        if not sandbox_env:
            raise HTTPExceptions.BAD_REQUEST.with_response(f"No environment with UUID '{sandbox_env_uuid}' found")
    ...
    if env is not None:
        ...
        with open(os.path.join(settings.ENVIRONMENT_DIR, uuid_env), "w+b") as f:
            f.write(env.read())
...
def parse_envvars(config: dict) -> Optional[dict]:
    """Check the validity of 'envvars' in the request and return it, returns None if it is not present."""
    if "envvars" in config:
        if not isinstance(config["envvars"], dict):
            raise HTTPExceptions.BAD_REQUEST.with_response(f'envvars must be an object, not {type(config["envvars"])}')
...
def parse_result_path(config: dict) -> Optional[str]:
    """Check the validity of 'result' in the request and return it, returns None if it is not present."""
    if "result_path" in config:
        if not isinstance(config["result_path"], str):
            return HTTPExceptions.BAD_REQUEST.with_response(
...
def parse_save(config: dict) -> bool:
    """Check the validity of 'save' in the request and return it, returns False if it is not present."""
    if "save" in config:
