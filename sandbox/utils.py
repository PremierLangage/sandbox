...
def parse_environ(config: dict) -> dict:
    ...
    if "environ" in config:
        ...
        return (
            {k: str(v) for k, v in config["environ"].items()}
            if "environ" in config
            else dict()
        )
    return {}
...
def specifications() -> dict:
    ...
    return {
        ...
        "container": {
            ...
            "memory": {"ram": ram, "swap": swap, "storage": container_storage_opt()},
            "io": {
...
def usage_io_network() -> Tuple[dict, dict]:
    ...
    raw_io1 = {
        disks[k]: v for k, v in psutil.disk_io_counters(True).items() if k in disks
    }
