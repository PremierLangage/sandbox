...
def execute(queue: Queue, test_data: dict, index: int):
    ...
    if environment is not None:
        data["environment"] = open(
            os.path.join(settings.ENVIRONMENT_ROOT, environment), "rb"
        )
...
class StressTestCase(SandboxTestCase):
    def test_stress_same_time(self):
        ...
        for i in range(count):
            ...
            threads.append(
                threading.Thread(target=execute, args=(queue, matrix[index], index))
            )
    ...
    def test_stress_over_time(self):
        ...
        for i in range(200):
            ...
            index = random.randint(0, n)
            t = threading.Thread(target=execute, args=(queue, matrix[index], index))
            t.start()
            threads.append(t)
            print(i + 1, "/", 200)
