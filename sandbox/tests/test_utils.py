...
class GetEnvTestCase(SandboxTestCase):
    def test_get_env_ok(self):
        self.assertEqual(
            os.path.join(TEST_ENVIRONMENT_ROOT, f"{ENV1}.tgz"), utils.get_env(ENV1)
        )
...
class ExecutedEnvTestCase(SandboxTestCase):
    ...
    def test_executed_env_only_sandbox(self):
        ...
        with open(env1_path, "rb") as env1, open(
            executed_env_path, "rb"
        ) as executed_env:
    ...
    def test_executed_env_only_body(self):
        ...
        executed_env_path = os.path.join(TEST_ENVIRONMENT_ROOT, "%s.tgz" % env_uuid)
        with open(env2_path, "rb") as env2, open(
            executed_env_path, "rb"
        ) as executed_env:
    ...
    def test_executed_env_sandbox_and_body(self):
        ...
        with tarfile.open(
            os.path.join(TEST_ENVIRONMENT_ROOT, "%s.tgz" % env_uuid), "r:gz"
        ) as tar:
            expected = {
                "",
                "dir",
...
class ContainerRamSwapTestCase(SimpleTestCase):
    ...
    @override_settings(
        DOCKER_PARAMETERS={
            **settings.DOCKER_PARAMETERS,
            **{"mem_limit": "100m", "memswap_limit": "0"},
        }
    )
    ...
    @override_settings(
        DOCKER_PARAMETERS={
            **settings.DOCKER_PARAMETERS,
            **{"mem_limit": "100m", "memswap_limit": "150m"},
        }
    )
...
class ContainerStorageOptTestCase(SimpleTestCase):
    ...
    def test_container_storage_opt_host_without_docker_parameter(self):
        self.assertEqual(-1, utils.container_storage_opt())
