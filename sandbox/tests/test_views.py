...
from sandbox.executor import SandboxErrorCode
...
class EnvViewTestCase(SandboxTestCase):
    def test_head_ok(self):
        ...
        self.assertEqual(
            response["Content-Disposition"], f"attachment; filename={ENV1}.tgz"
        )
    ...
    def test_head_404(self):
        response = self.client.head(
            reverse("sandbox:environment", args=(uuid.uuid4(),))
        )
...
class FileViewTestCase(SandboxTestCase):
    def test_head_ok(self):
    ...
    def test_head_404_env(self):
        response = self.client.head(
            reverse("sandbox:file", args=(uuid.uuid4(), "unknown"))
        )
...
class ExecuteTestCase(SandboxTestCase):
    ...
    def test_execute_ok_with_env_body_save(self):
        data = {
            ...
            "environment": open(
                os.path.join(settings.ENVIRONMENT_ROOT, f"{ENV1}.tgz"), "rb"
            ),
        }
        response = self.client.post(reverse("sandbox:execute"), data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_execute_timeout(self):
        ...
        self.assertEqual(SandboxErrorCode.TIMEOUT, result["execution"][1]["exit_code"])
    ...
    def test_execute_result_not_found(self):
        data = {
            "config": json.dumps({"commands": ["true"], "result_path": "unknown.txt"})
        }
        response = self.client.post(reverse("sandbox:execute"), data)
        self.assertEqual(response.status_code, 200)
